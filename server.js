...
app.post('/api/clear-pending-cash-orders', requireApiKey, async (req, res) => {
    try {
        console.log("⚠️ Received request to clear pending cash orders.");
        const result = await ordersCollection.deleteMany({
            paymentMethod: 'cash',
            $or: [
                { status: 'pending' },
                { status: 'cash_capture_pending' }
            ]
        });
        const message = `✅ Successfully cleared ${result.deletedCount} stuck cash order(s).`;
        console.log(message);
        res.status(200).send({ message });
    } catch (error) {
        console.error('Error clearing pending cash orders:', error);
        res.status(500).send({ message: 'Failed to clear pending cash orders.' });
    }
});
