...
if (!process.env.GEMINI_API_KEY) {
  console.warn("⚠️ GEMINI_API_KEY is not set. Cash verification will fail.");
}
if (!process.env.MONGODB_URI) {
    console.error("❌ MONGODB_URI is not set. The application cannot connect to the database.");
    process.exit(1);
}
if (!process.env.VENDING_API_KEY) {
    console.warn("⚠️ VENDING_API_KEY is not set. ESP32 communication will be insecure.");
}
...
const requireApiKey = (req, res, next) => {
    ...
    if (!apiKey || apiKey !== process.env.VENDING_API_KEY) {
...
app.get('/api/vending-command/:deviceId', requireApiKey, async (req, res) => {
    ...
    try {
        // 1. Check if there's a pending cash order waiting to be processed
        ...
        // 2. Check for a 'vend' command in the database
